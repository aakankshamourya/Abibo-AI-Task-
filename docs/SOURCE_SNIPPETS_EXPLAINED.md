...
2. **RAG pipeline** (`doc_intel_search.query_pdfs_intel`):
   ...
   - Sends those chunks + query to the LLM → gets the answer text
...
|------|------|-------------------|
| Citations are **created** | `doc_intel_search.py` | Around lines 206–222: loop over `docs`, build `citations` with `filename`, `label`, `description` (= chunk `page_content`) |
| Citations are **formatted** | `telegram_bot.py` | `_format_source_snippets()` (around lines 52–62): for each citation, take `filename` and `description`, truncate description to 180 chars, output `• filename — "snippet..."` |
...
3. In the reply you should see:
   ...
   - Then a block **"📎 Source snippets:"**
   - Under it, lines like:  
     **• leave_policy.pdf — "Employees are entitled to 12 days casual leave per year..."**
...
- Truncation is done in `_format_source_snippets` with `max_snippet_len=180`, cutting at a word boundary so we don’t break mid-word
